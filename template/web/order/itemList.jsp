            ...
            $("#query").click(function () {
                ...
                $.ajax({
                    ...
                    success: function (JData) {
                        $("#table_div").empty();
                        $("#table_div").append("<table id=\"detailTable\" class=\"display\" cellspacing=\"0\" width=\"100%\"><thead><tr><th>项目编号</th><th>客户编号</th><th>项目有效日（起）</th><th>项目有效日（讫）</th><th>项目状态</th><th>修改时间</th><th>修改人</th><th>操作</th> </tr></thead><tfoot></tfoot><tbody></tbody></table>");
                        $.each(JData, function (index, element) {
                            $("#detailTable > tbody ").append("<tr role=\"row\" ><td>" + element.projectCode + "</td><td>" + element.customerId + "</td><td>" + element.validFrom + "</td><td>" + element.validEnd + "</td><td>" + element.projectStatus + "</td><td>" + element.logTime + "</td><td>" + element.logUser + "</td><td><select  key='" + element.projectCode + "' inx=\"" + index + "\" class=\"form-control m-bot15\" id=\"operation\" name=\"operation\"> <option value=\"\">--</option><option value=\"C\">确定項目代码</option></select></td></tr>");
                        });
                        ...
                        $("#operation").change(function () {
                            if ($(this).val() === "C") {
                                $('#projectCode', opener.document).val($(this).attr("key"));
                                window.close();
                            }
                        });
                        $("#table_div").unmask();
